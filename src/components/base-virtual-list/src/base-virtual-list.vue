...
// 二分法去查找对应的index
function getStartIndex(scrollTop = 0): number {
  let low = 0
...
function getCurrentTop(index: number) {
...
onUpdated(() => {
  ...
  [...childrenList].forEach((node: any, index: number) => {
    ...
    if (state.cacheData[currentIndex].height === height)
      return
...
watchEffect(() => {
  clientData.value.forEach((_, index) => {
    const currentIndex = state.start + index
    if (Object.hasOwn(state.cacheData, currentIndex))
      return
...
<template>
  <div class="base-virtual-wrapper" :class="[className]" :style="getWrapperStyle" @scroll="onScroll">
